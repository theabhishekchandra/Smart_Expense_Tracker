...
import com.abhishek.spendly.data.local.room.entity.StaffEntity
import com.abhishek.spendly.data.local.room.entity.StaffExpenseSummary
import com.abhishek.spendly.data.local.room.entity.StaffWithAllocations
...
interface StaffDao {
    ...
    @Query("UPDATE staff SET isActive = 0 WHERE staffId = :id")
    suspend fun deactivateStaff(id: Long)

    @Query("UPDATE staff SET isActive = 1 WHERE staffId = :id")
...
//    @Query(
//        """
//        SELECT s.staffId as id, s.name as name, s.email as email, s.role as role, s.phone as phone, s.profilePicUri as profilePicUri, s.createdAt as createdAt, s.isActive as isActive,
//               IFNULL(SUM(e.amount), 0) as totalExpenses
//        FROM staff s
//        LEFT JOIN expenses e ON e.userId = s.staffId
//        GROUP BY s.staffId
